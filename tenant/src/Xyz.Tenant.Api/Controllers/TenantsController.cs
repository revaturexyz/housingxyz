...
namespace Revature.Tenant.Api.Controllers
{
  ...
  public class TenantsController : ControllerBase
  {
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<Revature.Tenant.Api.Models.ApiTenant>> GetByIdAsync([FromRoute] int id)
    {
      try
      {
        var tenant = await _tenantRepository.GetByIdAsync(id);//this is a repository function that should be async and return Task<IEnumerable<Revature.Tenant.Lib.Models.Tenant>>
